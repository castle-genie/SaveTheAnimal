$(function(){
	...
	$("#resetPwdBtn").on("click", function(){      
        ...
        if (!chkData("#userId", "아이디를") || !chkData("#userEmail", "이메일을")) return;	
        ...
        else if (findUserByIdAndEmail(userId, userEmail) == false) {
			...
			resetPwd(userId, userEmail, tempPassword); // 임시 비밀번호로 비밀번호 재설정하는 함수 호출
			if (resetPwd) {
				$(".inputArea").hide();
				$(".outputArea").show();
				$("#output").text(tempPassword);
			} else {
				$(".inputArea").show();
				$(".outputArea").hide();
			}
